...
$biaya_operasional_result = mysqli_query($koneksi, "SELECT jumlah FROM operasional where tgl_operasional = CURDATE()");
if ($biaya_operasional_result) {
    $biaya_operasional = mysqli_fetch_array($biaya_operasional_result);
} else {
    $biaya_operasional = null;
}

$pendapatan_sewa_result = mysqli_query($koneksi, "SELECT jumlah FROM pendapatan_sewa where tgl_pendapatan_sewa = CURDATE()");
if ($pendapatan_sewa_result) {
    $pendapatan_sewa = mysqli_fetch_array($pendapatan_sewa_result);
} else {
    $pendapatan_sewa = null;
}

$pendapatan_sewa_result = mysqli_query($koneksi, "SELECT * FROM pendapatan_sewa");
$arraymasuk = [];
if ($pendapatan_sewa_result) {
    while ($masuk = mysqli_fetch_array($pendapatan_sewa_result)) {
        $arraymasuk[] = $masuk['jumlah'];
    }
}
$jumlahmasuk = array_sum($arraymasuk);

$operasional_result = mysqli_query($koneksi, "SELECT * FROM operasional");
$arraykeluar = [];
if ($operasional_result) {
    while ($keluar = mysqli_fetch_array($operasional_result)) {
